...
thread_tick (void)
{
  ...
  if (t == idle_thread)
    idle_ticks++;
#ifdef USERPROG
  else if (t->pagedir != NULL)   // interesting...
    user_ticks++;
#endif
  else
    kernel_ticks++;
               ...
               thread_func *function, void *aux)
{
  ...
  /* Allocate thread. */
  t = palloc_get_page (PAL_ZERO);   // this allocates memory for thread stack (Yerrabali's slides)
  if (t == NULL) {
     tid = TID_ERROR;
     goto done;
 }
...
{
  // if the current thread donated its priority,
  // just update its priority, else only change its priority before dontation
...
/* Donate priority from a high-priority thread to a lower-priority thread */
void
thread_donate_priority(struct thread *donor, struct thread *receiver)
{
  // assign donor's priority to the reciever
...
/* Called every second to calculate load average */
void
 thread_calculate_load_avg (void)
 {
   int ready_threads;
   (thread_current () != idle_thread) ? (ready_threads = list_size (&ready_list) + 1) : (ready_threads = 0);
   load_avg = MUL_FP (CONVERT_FP (59) / 60, load_avg) + CONVERT_FP (1) / 60 * ready_threads;
}
...
{
  return CONVERT_INT_NEAR(100 * load_avg);
}
...
thread_get_recent_cpu(void)
{
  ...
  return CONVERT_INT_NEAR(100*thread_current()->recent_cpu);
}

